...
Instead of searching a fixed parameter box, the optimizer searches function
space directly: candidates are RBF mixtures from phase
...
def as_control_law(f: rkhs.RBFMixture) -> ControlLaw:
    ...
    def control_law(s: Float[np.ndarray, "... 1"]) -> Float[np.ndarray, "... J"]:
...
def minimize_functional(
    ...
    Starts from an LHS design of ``n_init`` single-atom mixtures and runs stages
    up to ``m`` atoms; total evaluations are ``n_init * 2**n_stages``.
    """
    ...
    # basis-size schedule: 1 doubling up to m
    schedule = [1 << s for s in range((m - 1).bit_length())] + [m]

    # evaluated LHS design at the coarsest basis
    key, key_init = jr.split(key)
    fs = rkhs.RBFMixture.from_lhs(
        key_init, (n_init, n_joints, 1, 1), l_range, x_range, a_range
    )
