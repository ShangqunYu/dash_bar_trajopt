...
from dash_mjlab.trajopt.example import hold_spawn, two_phase_push
...
def main() -> None:
    ...
    if args.out is None:
        ...
        args.out = Path(f"outputs/turn-{args.num_spokes}-spoke-to-{angle_str}")
    args.out.mkdir(parents=True, exist_ok=True)

    env = BarAngleTrajOptEnv(num_spokes=args.num_spokes)
    hold_cost, hold_traj = env.evaluate(
        hold_spawn, args.target_angle, return_trajectory=True
    )
    hold_cost = float(hold_cost)
    demo_cost = float(env.evaluate(two_phase_push, args.target_angle))
    n_stages = (args.m - 1).bit_length() + 1
    print(
        f"target {args.target_angle} rad, {args.num_spokes} spokes, "
