...
from dash_mjlab.trajopt.bofus import kernels, rkhs
...
def test_mixture_evaluation_matches_manual_rbf_sum():
    f = random_mixture(jax.random.key(0))
    t = jax.random.uniform(jax.random.key(1), (7, 3))

    d2 = jnp.sum((t[:, None, None] - f.x) ** 2 / f.l, axis=-1)
    manual = jnp.sum(f.a * jnp.exp(-0.5 * d2), axis=-1)
    assert f(t) == pytest.approx(manual, abs=1e-6)
